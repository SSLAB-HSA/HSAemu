...
#ifdef CONFIG_HSA
DEF("hsacu", HAS_ARG, QEMU_OPTION_hsacu,
    "-hsacu n[,maxcus=cus][,threads=threads]\n"
    "                set the number of CUs of the HSA unit to 'n' [default=1]\n"
    "                maxcus= maximum number of total cus\n"
    "                threads= number of threads runs the total cus\n",
    QEMU_ARCH_ALL)
#endif
STEXI
@item -hsacu @var{n}[,threads=@var{threads}][,maxcpus=@var{maxcpus}]
@findex -hsacu
Simulate an HSA arch system with @var{n} CUs.
For the HSA arch , the number of @var{threads} run the total number of @var{n} cus
ETEXI
