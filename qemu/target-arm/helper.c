...
void do_interrupt(CPUARMState *env)
{
    ...
    case EXCP_UDEF:
        ...
        if (env->thumb)
            ...
            offset = 4;
        break;
    case EXCP_SWI:  /* zdguo: use regs[15](pc) through mmu fetch the ins to get the imm */
        if (semihosting_enabled) {
        ...
        if (mask) {
            if (mask == 0x37) {    /* zdguo: 0xdf37 need to use a macro */
                env->regs[2] = (uint32_t)hsa_do_vgpu(env, (uint64_t)env->regs[1]);
            }else if (mask == 0x38) {
	    	env->regs[0] = hsa_remote_m2s(env, env->regs[1], env->regs[2]);
	    }
            return;
        }
#endif
